...
class App(Frame):
    ...
    def buildUI(self):
        ...
        self.machinevisionmenu.add_command(label="Ellipse detection", command=lambda:ellipseDetection())
...
def lineDetection():
    ...
    pixels = workingImage["pixels"]
    width, height = workingImage["size"][0], workingImage["size"][1]
    lines = filters.houghLines(pixels, width, height)
...
def circleDetection(radius=0):
    ...
    pixels = workingImage["pixels"]
    width, height = workingImage["size"][0], workingImage["size"][1]
    area, maxDiag = workingImage["area"],workingImage["maxDiag"]

    allCircles = filters.circleDetection(pixels, width, height, radius=radius)
...
def ellipseDetection():
    ...
    pixels, lines, points, origins, ellipses = filters.ellipseDetection(pixels, width, height)

    app.resetCanvas()
    saveImageChanges(pixels)
    for a, e in enumerate(ellipses):
        c = e["center"]
        minD, maxD = e["minD"], e["maxD"]
        minDp, maxDp = e["minSemiD"]*100/maxDiag, e["maxSemiD"]*100/maxDiag

        draw.line(minD, fill="red")
        draw.line(maxD, fill="green")
        draw.ellipse((c[0]-2, c[1]-2, c[0]+2, c[1]+2), fill="blue", outline="blue")
        draw.text((c[0]+2,c[1]+2), "ID%d"%(a), fill="white")
        draw.ellipse(e["box"], fill=None, outline="orange")
        print "Ellipse/Circulo [ID%d]\tArea (pixeles) = %d\t Area (porcentaje) = %d%%"%(a, e["size"], e["percentage"])
        #print "Ellipse [ID%d]\tSemidiametro menor = %d\t Semidiametro mayor = %d\t Menor.Percent = %d%%\t Menor.Percent = %d%%\n\n"%(a, e["minSemiD"], e["maxSemiD"], minDp, maxDp)
    #print "Lines = %d, points = %d"%(len(lines), len(points))
    #raw_input()
    #for a, l in enumerate(lines, start=1):
    #    print a-1
    #    draw.ellipse((origins[a-1][0]-2, origins[a-1][1]-2, origins[a-1][0]+2, origins[a-1][1]+2), fill="green", outline="green")
    #    draw.line(l, fill="blue")
    #    if(a%4 == 0):
    #        for p in points[(a/4)-1]:
                #print p
    #            draw.ellipse((p[0]-2, p[1]-2, p[0]+2, p[1]+2), fill="red", outline="red")
    #        saveImageDraw(mask)
    #        app.updateCanvas()
    #        raw_input()
    #        app.resetCanvas()
...
def imageToPixels(inputImage):
    ...
    for y in xrange(h):
        for x in xrange(w):
            pixel = tuple(pixels[x,y][:3])
            pixelsRGB.append(pixel)
    image["instance"] = i
    image["size"] = tuple([w, h])
    image["area"] = w * h
    image["maxDiag"] = int(round((w**2 + h**2)**0.5,0))
...
def saveImageDraw(mask):
    ...
    workingImage["area"] = w * h
    workingImage["maxDiag"] = int(round((w**2 + h**2)**0.5, 0))
